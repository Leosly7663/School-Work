...
{	
	...
	FILE *fp;
	FASTArecord fRecord;
	FASTArecord* fRecordAry = (FASTArecord*)malloc(capacity * sizeof(FASTArecord));
	...
	do {
		...
		* we know something is happening */
		...
		if (status == 0) {
			...
			fRecordAry = (FASTArecord*)realloc(fRecordAry, capacity * sizeof(FASTArecord));
			if (fRecordAry == NULL) {
	...
	int wastedMemory = (capacity - size) * sizeof(FASTArecord);
	// Calculate the percentage of wasted memory
	float wastedPercentage = (float)wastedMemory / (capacity * sizeof(FASTArecord)) * 100;

	// Print the wasted memory
	printf("Wasted memory: %d bytes (%.2f%% waste)\n", wastedMemory, wastedPercentage);
