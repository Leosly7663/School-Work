...
{	
	...
	FILE *fp;
	FASTArecord fRecord;
	// Declare an array of pointers to linked list heads
	LLvNode* fRecordAry = (LLvNode*)malloc(sizeof(FASTArecord));
	...
	do {
		...
		* we know something is happening */
		...
		if (status == 0) {
			...
			//TODO
			llAppend(fRecordAry,llNewNode((char *)fRecord.id,(void *) &fRecord));
